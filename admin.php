...
{
 		//setup a list of admin options
		//list, edit, add :  herbs, properties, energetics, ailments, formulas
		
		//admin.php?action=list&which=herb
		$list = '
		<tr>
		 <td colspan="5">LIST</td>
		</tr>
		<tr>
  		<td><a href="admin_herbs.php?action=list&which=herbs" >Herbs</a></td>
  		<td><a href="admin_properties.php?action=list&which=properties" >Properties</a></td>
  		<td><a href="admin_energetics.php?action=list&which=energetics" >Energetics</a></td>
		</tr>
		<tr>
  		<td><a href="admin_ailments.php?action=list&which=ailments" >Ailments</a></td>
  		<td><a href="admin_formulas.php?action=list&which=formulas" >Formulas</a></td>
  ...
  if (!is_null($herbInfo))
  {
  	foreach ($herbInfo as $id => $data)
  	{
	    ...
	    $list .= '<p class="' . $rowStyle . '">';
			$list .= '<a href="admin.php?action=edit&which=herb&id=' . $data['herbID'] . '" >';
   		$list .= $data['herb']. "&nbsp;";
			$list .= '</a><br/>';
			if ($data['latin_name'] != "")
			{
...
function listProperties()
{
  ...
  if (!is_null($propInfo))
  {
  	foreach ($propInfo as $id => $data)
  	{
  		$rowStyle == 'rowon' ? $rowStyle = 'rowoff' : $rowStyle = 'rowon';
	    $list .= '<p class="' . $rowStyle . '">';
			$list .= '<a href="admin.php?action=edit&which=property&id=' . $data['propertiesID'] . '" >';
   		$list .= $data['property']. "&nbsp;";
			$list .= '</a><br/>';	
...
<?
    ...
    $list .= '  		<td align="left">';
      ...
      $list .= '  			<FORM METHOD="POST" ACTION="admin.php?action=update">';
